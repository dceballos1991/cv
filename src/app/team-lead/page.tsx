...
import { RESUME_DATA_TEAM_LEAD } from "@/data/resume-data";
import { ProjectCard } from "@/components/project-card";

export const metadata: Metadata = {
  title: `${RESUME_DATA_TEAM_LEAD.name} | ${RESUME_DATA_TEAM_LEAD.about}`,
  description: RESUME_DATA_TEAM_LEAD.summary,
};
    ...
    <main className="container relative mx-auto scroll-my-12 overflow-auto p-4 print:px-3 print:py-0 md:p-16">
      <section className="mx-auto w-full max-w-3xl space-y-8 bg-white print:space-y-4">
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-between">
              ...
              <div className="hidden w-full gap-x-6 pt-0 font-mono text-sm text-muted-foreground print:flex print:text-[12px]">
                <p className="items-center text-pretty font-mono text-xs text-muted-foreground">
                  <a
                    ...
                    href={RESUME_DATA_TEAM_LEAD.locationLink}
                    target="_blank"
                  >
                    <GlobeIcon className="size-3" />
                    {RESUME_DATA_TEAM_LEAD.location}
                  </a>
                </p>
                {RESUME_DATA_TEAM_LEAD.contact.email ? (
                  <a
                    href={`mailto:${RESUME_DATA_TEAM_LEAD.contact.email}`}
                    className="inline-flex gap-x-1.5 align-baseline leading-none hover:underline"
                  >
                ...
                {RESUME_DATA_TEAM_LEAD.contact.tel ? (
                  <a
                    href={`tel:${RESUME_DATA_TEAM_LEAD.contact.tel}`}
                    className="inline-flex gap-x-1.5 align-baseline leading-none hover:underline"
                  >
              ...
              <div className="flex items-center gap-x-1 font-mono text-sm text-muted-foreground print:hidden">
                {RESUME_DATA_TEAM_LEAD.contact.email ? (
                  <Button
                    ...
                    asChild
                  >
                    <a href={`mailto:${RESUME_DATA_TEAM_LEAD.contact.email}`}>
                ...
                {RESUME_DATA_TEAM_LEAD.contact.tel ? (
                  <Button
                    ...
                    asChild
                  >
                    <a href={`tel:${RESUME_DATA_TEAM_LEAD.contact.tel}`}>
                      <PhoneIcon className="size-4" />
                    </a>
                  </Button>
                ) : null}
                {RESUME_DATA_TEAM_LEAD.contact.social.map((social) => (
                  <Button
                    key={social.name}
                    className="size-8"
                    variant="outline"
            ...
            <Avatar className="size-28 print:hidden">
              <AvatarImage
                alt={RESUME_DATA_TEAM_LEAD.name}
                src={RESUME_DATA_TEAM_LEAD.avatarUrl}
              />
              <AvatarFallback>{RESUME_DATA_TEAM_LEAD.initials}</AvatarFallback>
            </Avatar>
            {/* Web only section - End*/}
          </div>

          <p className="text-pretty pt-2 font-mono text-sm text-muted-foreground print:pt-0 print:text-[12px]">
            {RESUME_DATA_TEAM_LEAD.summary}
          </p>
        </div>
        <div className="flex gap-x-8">
          <div className="flex flex-[2] flex-col">
            <Section>
              ...
              {RESUME_DATA_TEAM_LEAD.work.map((work) => {
                return (
                  <>
          ...
          <div className="flex flex-1 shrink-0 flex-col">
            <Section className="scroll-mb-16">
              ...
              <div className="-mx-3 grid grid-cols-1 gap-3 print:grid-cols-1 print:gap-2 md:grid-cols-1 lg:grid-cols-1">
                {RESUME_DATA_TEAM_LEAD.projects.map((project) => {
                  return (
                    <ProjectCard
                      key={project.title}
                      title={project.title}
        ...
        {/* <Section>
          ...
          {RESUME_DATA_TEAM_LEAD.education.map((education) => {
            return (
        ...
        <Section className="print:hidden">
          ...
          <div className="flex flex-wrap gap-1">
            {RESUME_DATA_TEAM_LEAD.skills.map((skill) => {
              return (
                <Badge
      ...
      <CommandMenu
        links={[
          // {
          //   url: RESUME_DATA_TEAM_LEAD.personalWebsiteUrl,
          //   title: "Personal Website",
          // },
          ...RESUME_DATA_TEAM_LEAD.contact.social.map((socialMediaLink) => ({
