...
import { RESUME_DATA_TEAM_LEAD_FULL } from "@/data/resume-data";
import { ProjectCard } from "@/components/project-card";

export const metadata: Metadata = {
  title: `${RESUME_DATA_TEAM_LEAD_FULL.name} | ${RESUME_DATA_TEAM_LEAD_FULL.about}`,
  description: RESUME_DATA_TEAM_LEAD_FULL.summary,
};
  ...
  return (
    <main className="container relative mx-auto scroll-my-12 overflow-auto p-4 print:px-3 print:py-0 md:p-16">
      <section className="mx-auto w-full max-w-3xl space-y-8 bg-white print:space-y-4">
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-between">
            <div className="flex-1 space-y-1.5">
              <h1 className="text-2xl font-bold">
                {RESUME_DATA_TEAM_LEAD_FULL.name}
                <span className="inline max-w-lg text-pretty font-mono text-sm print:hidden">
                  &nbsp;-&nbsp;{RESUME_DATA_TEAM_LEAD_FULL.about}
                </span>
              </h1>

              <p className="max-w-lg text-pretty font-mono text-sm print:hidden">
                {RESUME_DATA_TEAM_LEAD_FULL.education[0].degree}&nbsp;-&nbsp;
                {RESUME_DATA_TEAM_LEAD_FULL.education[0].school}
              </p>
              {/* Print only section */}
              ...
              <div className="hidden w-full gap-x-6 pt-0 font-mono text-sm text-muted-foreground print:flex print:text-[12px]">
                <p className="items-center text-pretty font-mono text-xs text-muted-foreground">
                  <a
                    ...
                    href={RESUME_DATA_TEAM_LEAD_FULL.locationLink}
                    target="_blank"
                  >
                    <GlobeIcon className="size-3" />
                    {RESUME_DATA_TEAM_LEAD_FULL.location}
                  </a>
                </p>
                {RESUME_DATA_TEAM_LEAD_FULL.contact.email ? (
                  <a
                    href={`mailto:${RESUME_DATA_TEAM_LEAD_FULL.contact.email}`}
                    className="inline-flex gap-x-1.5 align-baseline leading-none hover:underline"
                  >
                ...
                {RESUME_DATA_TEAM_LEAD_FULL.contact.tel ? (
                  <a
                    href={`tel:${RESUME_DATA_TEAM_LEAD_FULL.contact.tel}`}
                    className="inline-flex gap-x-1.5 align-baseline leading-none hover:underline"
                  >
              ...
              <div className="flex items-center gap-x-1 font-mono text-sm text-muted-foreground print:hidden">
                ...
                {RESUME_DATA_TEAM_LEAD_FULL.contact.tel ? (
                  <Button
                    ...
                    asChild
                  >
                    <a href={`tel:${RESUME_DATA_TEAM_LEAD_FULL.contact.tel}`}>
                      <PhoneIcon className="size-4" />
                    </a>
                  </Button>
                ) : null}
                {RESUME_DATA_TEAM_LEAD_FULL.contact.social.map((social) => (
                  <Button
                    key={social.name}
                    className="size-8"
                    variant="outline"
            ...
            <Avatar className="size-28 print:hidden">
              <AvatarImage
                alt={RESUME_DATA_TEAM_LEAD_FULL.name}
                src={RESUME_DATA_TEAM_LEAD_FULL.avatarUrl}
              />
              <AvatarFallback>
                {RESUME_DATA_TEAM_LEAD_FULL.initials}
              </AvatarFallback>
            </Avatar>
          ...
          <p className="text-pretty pt-2 font-mono text-sm text-muted-foreground print:pt-0 print:text-[12px]">
            {RESUME_DATA_TEAM_LEAD_FULL.summary}
          </p>
        </div>
        <div className="flex gap-x-8">
          <div className="flex flex-[2] flex-col">
            <Section>
              ...
              {RESUME_DATA_TEAM_LEAD_FULL.work.map((work) => {
                return (
                  <>
          ...
          <div className="flex flex-1 shrink-0 flex-col">
            <Section className="scroll-mb-16">
              ...
              <div className="-mx-3 grid grid-cols-1 gap-3 print:grid-cols-1 print:gap-2 md:grid-cols-1 lg:grid-cols-1">
                {RESUME_DATA_TEAM_LEAD_FULL.projects.map((project) => {
                  return (
                    <ProjectCard
                      key={project.title}
                      title={project.title}
        ...
        {/* <Section>
          ...
          {RESUME_DATA_TEAM_LEAD_FULL.education.map((education) => {
            return (
        ...
        <Section className="print:hidden">
          ...
          <div className="flex flex-wrap gap-1">
            {RESUME_DATA_TEAM_LEAD_FULL.skills.map((skill) => {
              return (
                <Badge
      ...
      <CommandMenu
        links={[
          // {
          //   url: RESUME_DATA_TEAM_LEAD_FULL.personalWebsiteUrl,
          //   title: "Personal Website",
          // },
          ...RESUME_DATA_TEAM_LEAD_FULL.contact.social.map(
            (socialMediaLink) => ({
