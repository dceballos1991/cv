...
import { RONG_SHAO_DATA } from "@/data/rong-shao";

export const metadata: Metadata = {
  title: `${RONG_SHAO_DATA.name} | ${RONG_SHAO_DATA.about}`,
  description: RONG_SHAO_DATA.summary,
};
      ...
      <section className="mx-auto w-full max-w-3xl space-y-8 bg-white print:space-y-4">
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-between">
              ...
              <div className="hidden w-full gap-x-6 pt-0 font-mono text-sm text-muted-foreground print:flex print:text-[12px]">
                <p className="items-center text-pretty font-mono text-xs text-muted-foreground">
                  <a
                    ...
                    href={RONG_SHAO_DATA.locationLink}
                    target="_blank"
                  >
                    <GlobeIcon className="size-3" />
                    {RONG_SHAO_DATA.location}
                  </a>
                </p>
                {RONG_SHAO_DATA.contact.email ? (
                  <a
                    href={`mailto:${RONG_SHAO_DATA.contact.email}`}
                    className="inline-flex gap-x-1.5 align-baseline leading-none hover:underline"
                  >
                ...
                {RONG_SHAO_DATA.contact.tel ? (
                  <a
                    href={`tel:${RONG_SHAO_DATA.contact.tel}`}
                    className="inline-flex gap-x-1.5 align-baseline leading-none hover:underline"
                  >
              ...
              <div className="flex items-center gap-x-1 font-mono text-sm text-muted-foreground print:hidden">
                {RONG_SHAO_DATA.contact.email ? (
                  <Button
                    ...
                    asChild
                  >
                    <a href={`mailto:${RONG_SHAO_DATA.contact.email}`}>
                ...
                {RONG_SHAO_DATA.contact.tel ? (
                  <Button
                    ...
                    asChild
                  >
                    <a href={`tel:${RONG_SHAO_DATA.contact.tel}`}>
                      <PhoneIcon className="size-4" />
                    </a>
                  </Button>
                ) : null}
                {RONG_SHAO_DATA.contact.social.map((social) => (
                  <Button
                    key={social.name}
                    className="size-8"
                    variant="outline"
            ...
            {RONG_SHAO_DATA.avatarUrl && (
              <Avatar className="size-28 print:hidden">
                <AvatarImage
                  alt={RONG_SHAO_DATA.name}
                  src={RONG_SHAO_DATA.avatarUrl}
                />
                <AvatarFallback>{RONG_SHAO_DATA.initials}</AvatarFallback>
              </Avatar>
            )}
        ...
        <div className="flex gap-x-8 print:gap-x-4">
          ...
          <div className="flex flex-1 shrink-0 flex-col gap-y-4">
            {RONG_SHAO_DATA.education.length > 0 && (
              <Section>
                <h2 className="text-xl font-bold">Education</h2>
                <div className="-mx-3 grid grid-cols-1 gap-3 print:gap-2">
                  {RONG_SHAO_DATA.education.map((edu) => {
                    return (
                      <Card
                        key={edu.school}
            ...
            <Section>
              ...
              <div className="-mx-3 grid grid-cols-1 gap-3 print:gap-2">
                {RONG_SHAO_DATA.highlights.map((highlight) => {
                  return (
                    <Card
                      key={highlight.title}
                      className="flex flex-col overflow-hidden border border-muted p-3"
            ...
            <Section>
              ...
              <div className="flex flex-wrap gap-1">
                {RONG_SHAO_DATA.skills.map((skill) => {
                  return (
                    <Badge
